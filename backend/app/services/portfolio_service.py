...
class PortfolioService:
    ...
    def _process_daily_returns(data_length, history_data_path, daily_returns_path):
        ...
        CSVHandler.write_csv_all_columns(
            daily_returns_dict_asc, return_dates_desc, daily_returns_path
        )
    ...
    def optimize(
        ...
        for ticker in tickers:
            forecast = LSTMService.forecast_custom(
                ticker, data_length, control_length, optimizer, window_size, hidden_size
            )
        ...
        CSVHandler.add_to_csv_file(
            history_data_path, predictions_data_path, predictions_results, "control"
        )
        PortfolioService._process_daily_returns(
            data_length, predictions_data_path, daily_returns_path
        )

        optimizer = MarkowitzMethod(
            daily_returns_path, tickers, risk_threshold, rows_num
        )
